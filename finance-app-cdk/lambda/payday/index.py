...
def _get_upcoming(user_id, query_params):
    ...
    # A date already submitted for - show what actually happened (the
    # real transactions posted), but still include the freshly-computed
    # budgeted/planned reminders above. Checked even when the user
    # didn't explicitly browse to this date - viewing "your next
    # payday" after already submitting it needs to show the same
    # locked state, not silently allow a second submission.
    lookup_date = requested_date or next_payday
...
def _aggregate_by_external_account(due_expenses, external_account_names, current_estimate_fn, caller_user_id, account_names, budgeted_expenses=None, planned_items=None):
    ...
    Also includes budget and planned-expense transfers with a real
    destination account, grouped under that account with no external
    tag - those items don't carry an externalBankAccountId themselves
    (only recurring expense templates do), so they merge into whichever
    existing row (if any) already has no external tag for that account.
    ...
    for b in budgeted_expenses or []:
        if not b.get("accountId"):
            continue
        group_key = (b["accountId"], None)
        totals[group_key] = totals.get(group_key, 0) + b["amount"]
        labels.setdefault(group_key, {"accountName": account_names.get(b["accountId"], "Unknown account"), "externalAccountName": None})

    for pe in planned_items or []:
        if not pe.get("linkedAccountId"):
            continue
        group_key = (pe["linkedAccountId"], None)
        totals[group_key] = totals.get(group_key, 0) + pe["amount"]
        labels.setdefault(group_key, {"accountName": account_names.get(pe["linkedAccountId"], "Unknown account"), "externalAccountName": None})
