...
def _create(account_id, body):
    ...
    item = {
        ...
        "name": name[:100],
        "balance": decimal.Decimal(0),  # always starts at zero - a division's balance is derived entirely from real transactions tagged with it (deposits, expenses), never set directly, so it can never drift out of sync with what the account's own balance actually reflects
    }
...
def _update(account_id, division_id, body):
    ...
    # balance is deliberately not editable here - see the comment on
    # creation above. A division's balance only ever changes via a real,
    # signed transaction (adjust_division_balance), never a direct set.

    if not updates:
        return _response(400, {"error": "nothing to update - provide a new name"})
