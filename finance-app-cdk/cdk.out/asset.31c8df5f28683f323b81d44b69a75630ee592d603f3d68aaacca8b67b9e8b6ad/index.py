...
VALID_FREQUENCIES = {"weekly", "biweekly", "semimonthly", "monthly", "annual", "custom"}
...
def _create_recurring(user_id, access, account_id, body):
    ...
    if next_due_date < today and not body.get("keepAsOverdue"):
        for _ in range(120):
            next_due_date = next_date_after({"frequency": frequency, "anchorDate": body.get("anchorDate"), "anchorDays": body.get("anchorDays"), "anchorDay": body.get("anchorDay"), "anchorMonth": body.get("anchorMonth"), "intervalCount": body.get("intervalCount"), "intervalUnit": body.get("intervalUnit")}, next_due_date)
