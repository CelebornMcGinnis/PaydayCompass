...
def _create_recurring(user_id, access, account_id, body):
    ...
    requested_start = body.get("startDate") or date.today().isoformat()
    ...
    backfilled_count = 0
    if requested_start < today:
        # Generate every occurrence from the requested past start date up
        # to (not including) today, and write each as a real transaction
        # for trend visibility - explicitly NOT touching the account
        # balance, since the user is describing history that's already
        # reflected in their current real balance, not new money moving.
        # nextDueDate is set to the first occurrence ON OR AFTER today, so
        # the normal daily processor never re-touches these past dates.
        occurrence = requested_start
        ...
        item["nextDueDate"] = occurrence
    else:
        item["nextDueDate"] = requested_start
