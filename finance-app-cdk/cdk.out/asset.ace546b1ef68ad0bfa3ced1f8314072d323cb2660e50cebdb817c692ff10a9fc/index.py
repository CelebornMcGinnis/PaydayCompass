...
def _transfer(user_id, body):
    ...
    if from_account_id == to_account_id and from_division_id == to_division_id:
        return _response(400, {"error": "Choose a different account or a different division - this would move money to exactly where it already is."})
    ...
    for acct_id in (from_account_id, to_account_id):
        ...
        accounts[acct_id] = owned

    # Prevent going negative - checked against whichever balance the
    # money is actually coming out of: the division's own balance if
    # one was specified, otherwise the account's overall balance.
    if from_division_id:
        from_division = divisions_table.get_item(Key={"accountId": from_account_id, "divisionId": from_division_id}).get("Item")
        available = decimal.Decimal(str(from_division.get("balance", 0))) if from_division else decimal.Decimal(0)
        source_label = from_division.get("name", "that division") if from_division else "that division"
    else:
        available = decimal.Decimal(str(accounts[from_account_id].get("balance", 0)))
        source_label = accounts[from_account_id].get("name", "that account")
    if amount > available:
        return _response(400, {"error": f"That would take {source_label} negative - only {available} is available there."})
