...
from finance_common.payday_periods import next_real_payday_after as _next_real_payday_after, previous_real_payday as _previous_real_payday
...
def _get_upcoming(user_id, query_params):
    ...
    budgeted_expenses = [
        {
            "category": b["category"],
            "amount": budget_amount_due_on_payday(b, previous_payday, next_payday),
    ...
    # Everything due before the NEXT real payday still matters for this
    # decision - you're moving money now for the whole current pay
    # period, not just a fixed few days out. A fixed window (the
    # previous approach: payday + 5 days) silently excluded anything due
    # later in a longer pay period (e.g. a biweekly gap), which looked
    # like a bug even though the date math itself was correct.
    window_end = _next_real_payday_after(income_items, next_payday)
            ...
            "upcomingExpenses": [
                {
                    ...
                    "isAfterPayday": _effective_due_date(i) > next_payday,
...
def _aggregate_by_external_account(due_expenses, external_account_names, current_estimate_fn, caller_user_id, account_names):
    ...
    for item in due_expenses:
        ...
        label = f"{account_name} · drafted from {external_name}" if external_name else account_name
...
def _submit(user_id, body):
    ...
    if source_account_id:
        ...
        for b in active_budgets:
            ...
            if not dest:
                continue
            amount = budget_amount_due_on_payday(b, previous_payday, today)
    ...
    return _response(
        201,
        {"posted": posted, "transfers": transfers, "peerNotifications": notification_results},
        default=_decimal_default,
    )
