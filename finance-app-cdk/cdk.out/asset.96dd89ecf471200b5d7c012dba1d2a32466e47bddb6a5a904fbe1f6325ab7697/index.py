...
def _add_expense(user_id, access, account_id, body):
    ...
    with transactions_table.batch_writer() as batch:
        for split in splits:
            ...
            written.append(item)
            if division_id:
                adjust_division_balance(account_id, division_id, -split_amount)
    ...
    balance_result = accounts_table.update_item(
        ...
        ExpressionAttributeValues={":neg": -total_amount},
    ...
    notify_owner_of_shared_activity(
        owner_id, user_id, f"added a ${total_amount:.2f} expense ({', '.join(category_totals.keys())})"
    )
...
def _edit_purchase_splits(user_id, access, account_id, purchase_id, body):
    ...
    # Reverse every old row's division impact before deleting it.
    for row in existing_rows:
        if row.get("divisionId"):
            adjust_division_balance(account_id, row["divisionId"], decimal.Decimal(str(row["amount"])))
    ...
    # Only the NET difference moves the account balance - if the total
    # didn't change, this is a no-op regardless of how the splits shifted.
    net_delta = old_total - new_total
    if net_delta != 0:
        balance_result = accounts_table.update_item(
            Key={"userId": owner_id, "accountId": account_id},
            UpdateExpression="ADD balance :delta",
