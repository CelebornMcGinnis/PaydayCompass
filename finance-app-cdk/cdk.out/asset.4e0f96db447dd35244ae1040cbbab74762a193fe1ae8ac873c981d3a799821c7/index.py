...
def _post_occurrence(template, occurrence_date):
    ...
    # Per-template alert toggle: a user can turn off notifications for one
    # specific recurring transaction (e.g. a rent payment they never want
    # pinged about) without affecting alerts for anything else.
    if not is_income and template.get("notificationsEnabled", True):
