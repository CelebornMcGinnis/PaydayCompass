...
def execute_transfer(accounts_table, transactions_table, user_id, from_account_id, to_account_id, amount, description="", ts=None):
    """Moves real money between two of the user's own accounts: two
    linked transactions (a debit and a credit) plus the corresponding
    balance updates. Returns None if the accounts are the same or
    either account isn't found for this user - callers decide whether
    that's worth surfacing as an error or silently skipping (Payday's
    automatic transfers skip; the direct transfer endpoint errors)."""
