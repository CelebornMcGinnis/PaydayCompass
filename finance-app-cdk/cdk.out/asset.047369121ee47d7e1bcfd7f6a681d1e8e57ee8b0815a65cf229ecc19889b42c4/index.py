...
def _name_already_used(user_id, name, exclude_id=None):
    """Case-insensitive - matches the same check used for real Accounts."""
    existing = external_bank_accounts_table.query(
        KeyConditionExpression="userId = :uid",
        ExpressionAttributeValues={":uid": user_id},
    ).get("Items", [])
    ...
    return any(
        ...
        for a in existing
        if a["externalBankAccountId"] != exclude_id
    )
...
def _create(user_id, body):
    ...
    if _name_already_used(user_id, name):
        return _response(409, {"error": f'You already have an external account named "{name}" - pick a different name.'})
...
def _update(user_id, external_id, body):
    ...
    if not name:
        return _response(400, {"error": "name is required"})
    if _name_already_used(user_id, name, exclude_id=external_id):
        return _response(409, {"error": f'You already have an external account named "{name}" - pick a different name.'})
