...
from finance_common.planned_expenses import suggested_contribution as _suggested_contribution, complete_planned_expense as _complete_planned_expense, is_funded as _is_funded
...
def _update(user_id, planned_expense_id, body):
    ...
    new_item = None
    if body.get("completed") is not False and not updated.get("completed", False) and _is_funded(updated):
        new_item = _complete_planned_expense(planned_expenses_table, user_id, updated)
    ...
    return _response(200, {**updated, "newRolledOverItem": new_item}, default=_decimal_default)
...
def _mark_complete(user_id, planned_expense_id):
    ...
    new_item = _complete_planned_expense(planned_expenses_table, user_id, existing)
    if new_item:
        new_item["suggestedContribution"] = _suggested_contribution(new_item)
